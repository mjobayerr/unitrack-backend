...
async def test_a_product_can_be_created_and_appears_in_the_catalogue(client) -> None:
    ...
    listed = await client.get("/api/v1/admin/products")
...
async def test_withdrawing_a_product_hides_it_from_students_only(client) -> None:
    ...
    patched = await client.patch(f"/api/v1/admin/products/{product_id}", json={"active": False})
    assert patched.status_code == status.HTTP_200_OK
    assert patched.json()["active"] is False

    # Still visible to an operator, so it can be brought back.
    assert len((await client.get("/api/v1/admin/products")).json()) == 1
    assert (await client.get("/api/v1/admin/products?include_inactive=false")).json() == []
...
async def test_editing_a_price_does_not_clear_the_route_scope(client, db) -> None:
    ...
    patched = await client.patch(
        f"/api/v1/admin/products/{product_id}", json={"price_paisa": 95000}
    )
    assert patched.status_code == status.HTTP_200_OK
...
async def test_a_route_scope_can_still_be_cleared_deliberately(client, db) -> None:
    ...
    patched = await client.patch(f"/api/v1/admin/products/{product_id}", json={"route_scope": None})
...
async def test_a_stop_in_use_cannot_be_deleted(client, db) -> None:
    ...
    response = await client.delete(f"/api/v1/admin/stops/{stop.id}")
...
async def test_an_unused_stop_can_be_deleted(client, db) -> None:
    stop, = await _make_stops(db, 1)
    assert (await client.delete(f"/api/v1/admin/stops/{stop.id}")).status_code == 204
...
async def test_a_stop_can_be_moved(client, db) -> None:
    stop, = await _make_stops(db, 1)
    patched = await client.patch(
        f"/api/v1/admin/stops/{stop.id}", json={"lat": 23.8103, "lng": 90.4125}
    )
...
async def test_coordinates_off_the_planet_are_refused(client) -> None:
    response = await client.post(
        "/api/v1/admin/stops", json={"name": "Nowhere", "lat": 91, "lng": 0}
    )
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_CONTENT
...
async def test_a_name_may_repeat_only_in_the_other_direction(client) -> None:
    ...
    assert (await client.post("/api/v1/admin/routes", json=body)).status_code == 201

    assert (await client.post("/api/v1/admin/routes", json=body)).status_code == 409

    outbound = await client.post(
        "/api/v1/admin/routes", json={"name": "Uttara", "direction": "outbound"}
    )
...
async def test_stops_are_numbered_by_position(client, db) -> None:
    ...
    response = await client.put(
        f"/api/v1/admin/routes/{route.id}/stops",
        json={
            "stops": [
...
async def test_reordering_a_route_does_not_collide_with_itself(client, db) -> None:
    ...
    await client.put(f"/api/v1/admin/routes/{route.id}/stops", json=forward)

    reversed_order = {"stops": [{"stop_id": str(s.id)} for s in reversed(stops)]}
    response = await client.put(f"/api/v1/admin/routes/{route.id}/stops", json=reversed_order)
...
async def test_an_unknown_stop_leaves_the_existing_route_untouched(client, db) -> None:
    ...
    surviving = await client.get(f"/api/v1/fleet/routes/{route.id}")
...
async def test_retiring_a_route_hides_it_without_deleting_it(client, db) -> None:
    ...
    patched = await client.patch(f"/api/v1/admin/routes/{route.id}", json={"is_active": False})
    assert patched.json()["is_active"] is False

    assert (await client.get("/api/v1/fleet/routes")).json() == []
    assert (await client.get("/api/v1/fleet/routes?only_active=false")).json() != []
